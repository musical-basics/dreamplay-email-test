...
export function PreflightCheckCard({ subjectLine, htmlContent, variableValues }: PreflightCheckCardProps) {
  ...
  const detectedVariables = extractVariables(htmlContent)
  const hasUnsubscribeLink =
    htmlContent.toLowerCase().includes("unsubscribe") || htmlContent.includes("{{unsubscribe_url}}")

  // Build checklist items
  const checks: CheckItem[] = [
    {
      label: "Subject Line",
      status: subjectLine && subjectLine.trim().length > 0 ? "pass" : "fail",
      detail: subjectLine ? "Present" : "Missing",
    },
    {
  ...
  const missingValues = detectedVariables.filter((v) => !variableValues[v] || variableValues[v].trim() === "")
